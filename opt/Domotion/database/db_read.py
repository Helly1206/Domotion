...
class db_read(object):
    ...
    def FindProcessors(self, TimerId, SensorId, always = False):
        SearchDict = {}
        if always:
            SearchDict['Timer']=str(-1)
        if (TimerId > 0):
    ...
    def _UpdateDB130(self):
        self._AddRowAndSort("settings","Parameter","GPIO", "Id", (1, 'MQTT', 'Use MQTT (restart required)', 1, 'BOOL', 1))
        self._AddRowAndSort("settings","Parameter","Status_green", "Id", (1, 'MQTT_brokeraddress', 'MQTT broker hostname or IP address (restart required)', "some.broker.net", 'STRING', 1))
        self._AddRowAndSort("settings","Parameter","MQTT_brokeraddress", "Id", (1, 'MQTT_port', 'MQTT port (restart required)', 1883, 'INT', 1))
        self._AddRowAndSort("settings","Parameter","MQTT_port", "Id", (1, 'MQTT_username', 'MQTT username or leave empty (restart required)', "", 'STRING', 1))
        self._AddRowAndSort("settings","Parameter","MQTT_username", "Id", (1, 'MQTT_password', 'MQTT password or leave empty (restart required)', "", 'PSTRING', 1))
        self._AddRowAndSort("settings","Parameter","MQTT_password", "Id", (1, 'MQTT_QOS', 'MQTT quality of service (0,1 or 2)', 1, 'INTQOS', 0))
