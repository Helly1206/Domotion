...
class DomoWebStarter(object):
    ...
    def run(self, argv):
        ...
        if Debug:
            ...
            index = 1
            self.procs.append(subprocess.Popen(["python3", self.GetDomoWeb(), "-p 5000"]))
        else:
            ...
            for child in root:
                ...
                if ssl:
                    ...
                    self.procs.append(subprocess.Popen(["python3", self.GetDomoWeb(), "-p {p}".format(p=port), "-s", "-c {c}".format(c=cert), "-k {k}".format(k=key)]))
                else:
                    print(("Starting http server [%s] on port: %d"%(name,port)))
                    self.procs.append(subprocess.Popen(["python3", self.GetDomoWeb(), "-p {p}".format(p=port)]))
