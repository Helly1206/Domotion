...
class fuel(object):
    ...
    def process(self, timer, sensor, value):
        ...
        for proc in procs:
            ...
            if ((self._arithmic(procprop['SensorProcessor'], sensor, value)) or ((procprop['Timer'] == timer) and (timer))):
                self.scripts.run(procprop['Script'])
                self._combine(procprop['Combiner'])
        return

    def processAlways(self):
        for procprop in self.alwaysProcs:
            self.scripts.run(procprop['Script'])
            #self._combine(procprop['Combiner'])
    ...
    def GetSensorId(self, queueresult):
        ...
        elif (self.commandqueue.hardware(queueresult) ==  "MQTT"):
            Id = self.localaccess.FindSensorbyURL(self.commandqueue.device(queueresult), self.commandqueue.tag(queueresult))
            if (Id == 0):
                self.logger.info("Received unknown input from MQTT: topic: %s/%s", self.commandqueue.device(queueresult), self.commandqueue.tag(queueresult))
    ...
    def _DoSetActuator(self, props, value):
        ...
        if (props['Type'] == 2): # RF Output
            ...
            if (self.gpio):
                ...
                setvalue = True
        elif (props['Type'] == 15): # MQTT output
            if (self.mqtt):
                retval = self.mqtt.SetActuator(props['DeviceURL'], props['KeyTag'], value)
                setvalue = True
    ...
    def UpdateAlways(self):
        self.alwaysProcs = []
        procs = self.localaccess.FindProcessors(0, 0, True)
        for proc in procs:
            self.alwaysProcs.append(self.localaccess.GetProcessorProperties(proc))
